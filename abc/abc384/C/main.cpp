...
class Solver {
    private:
        ...
        stringstream ss;
        int a, b, c, d, e;
        map<int, set<string>, greater<int>> result;
...
void Solver::solve()
{
    ...
    while(idx <= 0b11111) {
        int score = 0;
        string name = "";
        if( (idx & 0b10000 ) != 0 ) { score += a; name += "A"; }
        if( (idx & 0b01000 ) != 0 ) { score += b; name += "B"; }
        if( (idx & 0b00100 ) != 0 ) { score += c; name += "C"; }
        if( (idx & 0b00010 ) != 0 ) { score += d; name += "D"; }
        if( (idx & 0b00001 ) != 0 ) { score += e; name += "E"; }
        result[score].insert(name);
        idx++;
    }
    for (const auto& [score, names] : result) {
